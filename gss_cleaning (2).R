...
raw_data <- read_csv("AAwXWyt3.csv")
...
gss$age = floor(gss$age)

gss$education <- 
  revalue(gss$education, c("Bachelor's degree (e.g. B.A., B.Sc., LL.B.)"="Bachelor's degree"))
gss$education <- 
  revalue(gss$education, c("Trade certificate or diploma"="University certificate or diploma below the bachelor's level"))
gss$education <- 
  revalue(gss$education, c("University certificate or diploma below the bachelor's level"="College, CEGEP or other non-university certificate or di..."))

gss <- gss%>%
  filter(education!="NA")

gss<- gss%>%
  dplyr::count(age,sex,province,education)%>%
  group_by(age,sex,province,education)


gss <- write_csv(gss, "gss.csv")
